  ...
  campaigns: [
    {
      ...
      caption: 'The future of drifting is here. ðŸŽï¸ðŸ’¨ #NeonDrift #ReelEarn',
    ...
    },
    {
      ...
      caption: 'Grind now, shine later. ðŸ’ª #FitnessHustle #ReelEarn',
    ...
    },
    {
      ...
      caption: 'Best street food in the city! ðŸ¥˜ðŸ”¥ #FoodHunt #ReelEarn',
...
useEffect(() => {
  ...
  const WalletView = () => {
    ...
    const handleWithdrawal = () => {
      ...
      if (!amount || amount < appState.config.minWithdrawal) return showToast(`Min withdrawal â‚¹${appState.config.minWithdrawal}`, 'error');
      ...
      addLog('payout', `Requested withdrawal â‚¹${amount}`, currentUser?.id, currentUser?.username);
    ...
    return (
      <div className="space-y-10 pb-40 animate-slide">
        ...
        <div className="glass-panel p-10 rounded-[56px] border-t-8 border-cyan-500 shadow-2xl">
          ...
          <h2 className="text-6xl font-black italic text-white mb-6">â‚¹{currentUser?.walletBalance.toLocaleString()}</h2>
          <div className="flex gap-4">
            <div className="flex-1 p-5 bg-white/5 rounded-3xl text-center"><p className="text-[8px] font-black text-slate-500 uppercase mb-1">Locked (Audit)</p><p className="text-lg font-black text-white italic">â‚¹{currentUser?.pendingBalance.toLocaleString()}</p></div>
            <div className="flex-1 p-5 bg-white/5 rounded-3xl text-center"><p className="text-[8px] font-black text-slate-500 uppercase mb-1">Total Earned</p><p className="text-lg font-black text-cyan-400 italic">â‚¹{currentUser?.totalEarnings.toLocaleString()}</p></div>
          </div>
        </div>
           ...
           <div className="space-y-8">
              <div className="glass-panel p-10 rounded-[56px] space-y-6 shadow-2xl">
                 ...
                 <div className="space-y-4">
                    <input type="number" value={withdrawAmount} onChange={e => setWithdrawAmount(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-2xl font-black text-cyan-400 italic outline-none shadow-inner" placeholder="Amount â‚¹" />
        ...
        {adminTab === 'dashboard' && (
          <div className="space-y-6 animate-slide">
            <div className="grid grid-cols-2 gap-4">
               ...
               <div className="glass-panel p-6 rounded-[32px] border-t-4 border-green-500 shadow-xl"><p className="text-[8px] font-black text-slate-500 uppercase italic">User Wallet Pool</p><p className="text-2xl font-black text-white italic">â‚¹{stats.totalBalance.toLocaleString()}</p></div>
               <div className="glass-panel p-6 rounded-[32px] border-t-4 border-orange-500 shadow-xl"><p className="text-[8px] font-black text-slate-500 uppercase italic">Capital In Audit</p><p className="text-2xl font-black text-white italic">â‚¹{stats.totalPending.toLocaleString()}</p></div>
               <div className="glass-panel p-6 rounded-[32px] border-t-4 border-red-500 shadow-xl"><p className="text-[8px] font-black text-slate-500 uppercase italic">Withdrawal Queue</p><p className="text-2xl font-black text-white italic">â‚¹{stats.withdrawalRequestsAmount.toLocaleString()}</p></div>
            </div>
            <div className="glass-panel p-10 rounded-[48px] border-l-8 border-cyan-500 shadow-[0_20px_60px_rgba(0,0,0,0.5)] relative overflow-hidden"><p className="text-[10px] font-black text-slate-500 uppercase mb-2 italic">Remaining Network Liquidity</p><h2 className="text-5xl font-black italic text-cyan-400">â‚¹{stats.cashflowRemaining.toLocaleString()}</h2><p className="text-[8px] font-black text-slate-700 uppercase mt-4">Node Capacity Cap: â‚¹{stats.cashflowLimit.toLocaleString()}</p></div>
          </div>
        )}
           ...
           <div className="space-y-6 animate-slide"> {appState.users.filter(u => u.role !== UserRole.ADMIN).map(u => (
              <div key={u.id} className="glass-panel p-8 rounded-[40px] space-y-4 shadow-xl border border-white/5">
                 <div className="flex justify-between items-center" onClick={() => setSelectedUserDetail(u)}>
                    ...
                    <p className="font-black text-white italic text-lg">â‚¹{u.walletBalance.toLocaleString()}</p>
                 </div>
        ...
        {adminTab === 'cashflow' && (
           <div className="space-y-10 animate-slide">
              <div className="glass-panel p-10 rounded-[48px] border-t-8 border-cyan-500 space-y-8 shadow-2xl">
                 ...
                 <div className="grid grid-cols-2 gap-4">
                    <div className="p-6 bg-white/5 rounded-3xl border border-white/5 text-center shadow-inner"><p className="text-[8px] font-black text-slate-500 uppercase mb-1 italic">Total Daily Cap</p><p className="text-xl font-black text-white italic">â‚¹{appState.cashflow.dailyLimit.toLocaleString()}</p></div>
                    <div className="p-6 bg-white/5 rounded-3xl border border-white/5 text-center shadow-inner"><p className="text-[8px] font-black text-slate-500 uppercase mb-1 italic">Locked for Audit</p><p className="text-xl font-black text-orange-400 italic">â‚¹{stats.pendingCashflow.toLocaleString()}</p></div>
                 </div>
                 <div className="p-10 bg-black/20 rounded-3xl border border-white/5 text-center shadow-2xl border-cyan-500/20"><p className="text-[10px] font-black text-slate-500 uppercase mb-2 italic">Network Available Burn</p><p className="text-5xl font-black text-cyan-400 italic">â‚¹{stats.cashflowRemaining.toLocaleString()}</p></div>
                 <div className="space-y-4">
                    <p className="text-[10px] font-black text-slate-600 uppercase px-4 italic">Modify System Threshold (â‚¹)</p>
                    ...
                    <button className="w-full btn-primary py-7 rounded-[32px] font-black uppercase text-sm shadow-2xl active:scale-95 transition-all" onClick={() => { showToast('Liquidity Profile Synchronized','success'); addLog('admin', `Daily Liquidity updated to â‚¹${appState.cashflow.dailyLimit}`); }}>Sync System Threshold</button>
                 </div>
              </div>
           </div>
        )}
                   ...
                   {appState.payoutRequests.filter(p => p.status === PayoutStatus.PENDING).map(r => (
                      <div key={r.id} className="glass-panel p-6 rounded-[32px] flex flex-col gap-4 shadow-xl border border-white/5 animate-slide">
                         <div className="flex justify-between items-center" onClick={() => setSelectedUserDetail(appState.users.find(u => u.id === r.userId) || null)}>
                            <div><p className="text-xl font-black text-white italic tracking-tighter">â‚¹{r.amount.toLocaleString()}</p><p className="text-[8px] text-slate-500 uppercase font-black tracking-widest">@{r.username} â€¢ {r.method}</p></div>
                  ...
                  {appState.submissions.filter(s => s.status === SubmissionStatus.PENDING || s.status === SubmissionStatus.VIRAL_CLAIM).map(sub => (
                    <div key={sub.id} className="glass-panel p-6 rounded-[32px] space-y-4 border border-white/5 shadow-xl animate-slide">
                       <div className="flex justify-between items-center" onClick={() => setSelectedUserDetail(appState.users.find(u => u.id === sub.userId) || null)}>
                          <div className="max-w-[70%]"><p className="text-xs font-black text-white italic uppercase tracking-tighter truncate">{sub.campaignTitle}</p><p className="text-[8px] text-slate-500 uppercase font-black">@{sub.username} â€¢ {sub.platform}</p></div>
                          <p className={`text-lg font-black italic ${sub.status === SubmissionStatus.VIRAL_CLAIM ? 'text-cyan-400' : 'text-slate-200'}`}>â‚¹{sub.rewardAmount}</p>
                       </div>
                       ...
                       <div className="flex gap-2">
                          <button onClick={() => { setAppState(p => ({...p, submissions: p.submissions.map(s => s.id === sub.id ? {...s, status: SubmissionStatus.APPROVED} : s), users: p.users.map(u => u.id === sub.userId ? {...u, walletBalance: u.walletBalance + sub.rewardAmount, pendingBalance: u.pendingBalance - sub.rewardAmount, totalEarnings: u.totalEarnings + sub.rewardAmount} : u)})); addLog('verify', `Mission Verified for @${sub.username} (+â‚¹${sub.rewardAmount})`, sub.userId, sub.username); showToast('Audit Verified: Capital Unlocked','success'); }} className="flex-1 py-3 bg-cyan-500 text-black rounded-xl text-[9px] font-black uppercase shadow-lg shadow-cyan-500/20 active:scale-95">Approve & Settle</button>
    ...
    return (
      <div className="fixed inset-0 z-[200] bg-black/95 backdrop-blur-3xl flex flex-col p-8 overflow-y-auto animate-slide">
        ...
        <div className="glass-panel p-10 rounded-[64px] border-t-8 border-cyan-500 shadow-2xl space-y-10 relative">
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="p-6 bg-white/5 rounded-3xl text-center shadow-inner"><p className="text-[8px] font-black text-slate-500 uppercase mb-1 tracking-widest italic">Capital Balance</p><p className="text-xl font-black text-white italic">â‚¹{selectedUserDetail.walletBalance.toLocaleString()}</p></div>
                <div className="p-6 bg-white/5 rounded-3xl text-center shadow-inner"><p className="text-[8px] font-black text-slate-500 uppercase mb-1 tracking-widest italic">Mission Total</p><p className="text-xl font-black text-cyan-400 italic">â‚¹{selectedUserDetail.totalEarnings.toLocaleString()}</p></div>
            </div>
            <div className="space-y-4"> 
            ...
            <div className="space-y-6 pt-4">
               <div className="space-y-1">
                  ...
                  <div className="bg-white/5 border border-white/10 rounded-2xl p-5 flex justify-between items-center shadow-inner group">
                     <p className="text-sm font-black text-white italic tracking-widest">{showKey ? currentUser.securityKey : 'â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢'}</p>
               ...
               <div className="space-y-1">
                  ...
                  <div className="bg-white/5 border border-white/10 rounded-2xl p-5 shadow-inner">
                     <p className="text-[10px] font-black text-white italic uppercase tracking-tighter">{currentUser.payoutMethod || 'None'} â€¢ {currentUser.payoutDetails || 'No details set'}</p>
                  </div>
               </div>
            ...
            {appState.campaigns.filter(c => c.active).map(c => (
              <div key={c.id} onClick={() => setSelectedCampaign(c)} className="glass-panel rounded-[56px] overflow-hidden relative group aspect-[9/16] shadow-[0_40px_100px_rgba(0,0,0,0.6)] cursor-pointer border border-white/5">
                ...
                <div className="absolute top-6 left-6 flex flex-col gap-2 z-20">
                   <div className="bg-black/90 backdrop-blur-md px-4 py-2 rounded-2xl border border-white/10 shadow-xl"><p className="text-[8px] font-black text-slate-500 uppercase italic">Basic Reward</p><p className="text-xl font-black text-white italic leading-tight">â‚¹{c.basicPay.toLocaleString()}</p></div>
                   <div className="bg-cyan-500/90 backdrop-blur-md px-4 py-2 rounded-2xl shadow-xl border border-white/20"><p className="text-[8px] font-black text-black/60 uppercase italic">Viral Bonus Pool</p><p className="text-xl font-black text-black italic leading-tight">â‚¹{c.viralPay.toLocaleString()}</p></div>
                </div>
